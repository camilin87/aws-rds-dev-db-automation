...
module.exports = {
    updateConnectionString: async (config) => {
        ...
        const updateResult = await elasticBeanstalkHelper(config.region)
            .updateEnvironmentSetting(config.environmentName, config.connectionStringKey, connectionString)
